...
```powershell
# 1. 准备构建 JDK：GTNHGradle 2.0.20 要求 JVM 25+。
#    把本机 JDK 路径写进用户级配置（不入库，CI 由 setup-java 提供）：
#    ~/.gradle/gradle.properties:
#      org.gradle.java.installations.auto-download = false
#      org.gradle.java.installations.paths = <zulu25 路径>,<zulu8 路径>,<zulu17 路径>
...
AE2（GTNH 分支 rv3-beta-695-GTNH）编译依赖从 GTNH Maven
（`com.github.GTNewHorizons:Applied-Energistics-2-Unofficial:rv3-beta-695-GTNH`）自动解析，
无需手动放置 jar；运行时由 GTNH 实例提供。

产物：`build/libs/gtnhlabeledwireless-<version>.jar`（正式版为无 `-dev` 后缀的 jar）。

> 注意：本机 JDK 路径已从仓库 `gradle.properties` 移入用户级 `~/.gradle/gradle.properties`，
> 换机器构建时只需修改用户级文件，无需改动仓库。
