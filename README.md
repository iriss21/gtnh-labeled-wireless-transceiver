...
### v0.5.1（当前）
- **修复邻接收发器刷物品漏洞**：两个不同频道的收发器紧贴放置时，AE2 的邻接节点检测（FindConnections）会错误地自动连接它们的网格节点 → 网格合并 → 两个标签网络的存储缓存合并 → 取物只减一侧 → 无限复制。v0.5.1 在 `getGridNode(ForgeDirection)` 中检查临街方块是否是另一个无线收发器，若是则返回 null 阻止自动连接，线缆连接不受影响。修复后收发器只能通过线缆接入 ME 网络，不能通过方块邻接直接互联。
- 特性变更：v0.5.0 三项功能未变。
