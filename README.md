...
### v0.5.11（当前）
- 修复重命名后所有收发器（含发起重命名的 GUI 所在收发器）仍显示旧名：重命名传播改为「网络自身端点表 + 全维度扫描」双路径且每个端点单独容错——此前单个端点的异常会中断整轮传播，导致注册表已改名（列表只剩新名）但各收发器仍显示旧名
- 重命名传播改为采纳新标签（adoptRenamedLabel，保持原网络与频道号），不再因归属派生不一致而把收发器误移到同标签的其他网络
- 新增频道号（channel）自愈兜底：收发器定期按频道号核对真实网络标签，漏改 / 归属派生漂移的端点自动采纳网络当前名，不依赖 ServerUtilities 队伍查询的时序一致性

### v0.5.10
- 修复重命名后关闭 GUI 重开仍显示旧名：重命名跟随改用频道号（channel）匹配在线端点，不再依赖标签字符串或端点坐标维度猜测，任意维度/任意标签表示形式下收发器都确定跟随新名
